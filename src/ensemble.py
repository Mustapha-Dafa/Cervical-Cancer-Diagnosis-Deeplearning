...
def doFusion_single(res1,res2,res3,class_no):
  
    rank1 = generateRank1(res1[0],class_no)*generateRank2(res1[0],class_no)
    rank2 = generateRank1(res2[0],class_no)*generateRank2(res2[0],class_no)
    rank3 = generateRank1(res3[0],class_no)*generateRank2(res3[0],class_no)
    rankSum = rank1 + rank2 + rank3
    rankSum = np.array(rankSum)
    scoreSum = 1 - (res1[0] + res2[0] + res3[0])/3
    scoreSum = np.array(scoreSum)
     
    fusedScore = (rankSum.T)*scoreSum
    cls = int(np.argmin(rankSum))
